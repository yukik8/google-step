...
class HashTable:
    ...
    def put(self, key, value, rehash=False):  # rehashの場合はchecksizeを実行しないように
        assert type(key) == str
        if not rehash:
        ...
        while item:
            if item.key == key:
                ...
                # 　削除対象が先頭以外だった場合
                    ...
                    # １つ前のnextを、次の次のノードに指定することで削除対象をスキップ
                    prev.next = item.next
                self.item_count -= 1
                return True
            # 各変数を１つ先に進める
    ...
    def rehash(self, new_size):
        ...
        self.buckets = [None] * self.bucket_size
        self.item_count = 0
        # コピーしたデータを新しいbucketにコピー
        for item in old_buckets:
            while item:
                self.put(item.key, item.value, True)
                item = item.next
