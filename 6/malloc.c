...
{
  // 　配列にしてBIN個のbinを管理
  my_metadata_t *free_head[BIN];
  my_metadata_t dummy[BIN];
} my_heap_t;
...
void my_remove_from_free_list(my_metadata_t *metadata, my_metadata_t *prev, int i)
{
  if (prev)
  {
...
void *my_malloc(size_t size)
{
  ...
  for (int i = bin; i < BIN; i++)
  {
    ...
    while (metadata)
    {
      ...
      {
        // 最小サイズのmetadataにbest_fitを更新
        if (best_fit == NULL || metadata->size < best_fit->size)
        {
          best_fit = metadata;
          best_fit_prev = prev;
          best_fit_bin = i;
  ...
  my_remove_from_free_list(best_fit, best_fit_prev, best_fit_bin);
